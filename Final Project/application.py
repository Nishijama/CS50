...
def index():
    ...
    if request.method == "GET":
        ...
        statisticsQuery = db.execute("SELECT * FROM statistics "
                                     "WHERE user_id = ? AND date BETWEEN ? AND ? "
                                     "ORDER BY date", user_id, first_of_month, last_of_month)

        topStatQuery = db.execute("Select SUM(requests) AS rTotal, "
                                  "ROUND(AVG(requests),2) AS rAverage, "
                                  "SUM(pages) AS pTotal, "
                                  "ROUND(AVG(pages),2) AS pAverage "
                                  "FROM statistics "
                                  "WHERE user_id = ? AND date BETWEEN ? AND ? "
                                  "ORDER BY date", user_id, first_of_month, last_of_month)

        shiftsQuery = db.execute("SELECT COUNT(user_id), shift "
                                 "FROM statistics "
                                 "WHERE shift IS NOT NULL AND user_id = ? AND date BETWEEN ? AND ? "
                                 "GROUP BY shift", user_id, first_of_month, last_of_month)

        tasksQuery = db.execute("SELECT COUNT(user_id), task "
                                "FROM statistics "
                                "WHERE task IS NOT NULL AND user_id = ? AND date BETWEEN ? AND ? "
                                "GROUP BY task", user_id, first_of_month, last_of_month)
    ...
    # Define POST request
        ...
        if "AddRecordsButton" in request.form:
            ...
            check_stats = db.execute("SELECT requests FROM statistics "
                                     "WHERE user_id = ? AND date BETWEEN ? AND ?", user_id, first_of_month, last_of_month)
            ...
            if len(check_stats) == 0:
                ...
                for dt in daterange(start_date, end_date):
                    ...
                    print(single_date)
                    db.execute("INSERT INTO statistics (user_id, date, requests, pages, shift, task) "
                               "VALUES (:user_id, :date, NULL, NULL, NULL, NULL)", user_id=user_id, date=single_date)

            # Update given date with new records
            db.execute("UPDATE statistics "
                       "SET requests = ?, pages = ?, shift = ?, task = ? "
                       "WHERE user_id = ? AND date = ?", requests, pages, shift, task, user_id, date_var)
        ...
        # Define Remove POST request
            ...
            check_stats = db.execute("SELECT requests FROM statistics "
                                     "WHERE user_id = ? AND date = ?", user_id, deleteQuery[:10])

            # Overwrite the records with NULL values
            if len(check_stats) != 0:
                db.execute("UPDATE statistics "
                           "SET requests = NULL, pages = NULL, shift = NULL, task = NULL "
                           "WHERE user_id = ? AND date = ?", user_id, deleteQuery[:10])
        ...
        topStatQuery = db.execute("Select SUM(requests) AS rTotal, "
                                  "ROUND(AVG(requests),2) AS rAverage, SUM(pages) AS pTotal, "
                                  "ROUND(AVG(pages),2) AS pAverage FROM statistics "
                                  "WHERE user_id = ? AND date BETWEEN ? AND ? "
                                  "ORDER BY date", user_id, first_of_month, last_of_month)

        shiftsQuery = db.execute("SELECT COUNT(user_id), shift FROM statistics "
                                 "WHERE shift IS NOT NULL AND user_id = ? AND date BETWEEN ? AND ? "
                                 "GROUP BY shift", user_id, first_of_month, last_of_month)

        tasksQuery = db.execute("SELECT COUNT(user_id), task FROM statistics "
                                "WHERE task IS NOT NULL AND user_id = ? AND date BETWEEN ? AND ? "
                                "GROUP BY task", user_id, first_of_month, last_of_month)
        ...
        # Return template
        return render_template("index.html", statisticsQuery=statisticsQuery, topStatQuery=topStatQuery, shiftsQuery=shiftsQuery,
                               tasksQuery=tasksQuery, year=year, month=month, statistics_chart_requests_data=statistics_chart_requests_data,
                               statistics_chart_pages_data=statistics_chart_pages_data, tasks_chart_data=tasks_chart_data,
                               shifts_chart_data=shifts_chart_data, topStatReqTotal=topStatReqTotal, topStatReqAvg=topStatReqAvg,
                               topStatPageTotal=topStatPageTotal, topStatPageAvg=topStatPageAvg)
...
# The code below is heavily leaning on implementation provided by CS50 for Finance Project
